...
int canalMain;                                                   // * Variable n° de canal
int canalProg;                                                   // * Variable n° de canal
int dp;
void IRAM_ATTR interrupt(rmt_channel_t channel, void *t) {
#ifdef RailCom_enable                                            // * Si RailCom autorisé
  if (channel == 0) {                                            // * Main
    gpio_matrix_out(GPIO_NUM_12, 0x100, false, false);           // * Déconnecte la pin 12 du module RMT
    gpio_set_level(GPIO_NUM_12, 1);                              // * Pin 12 à l'état haut
    canalMain = channel;                                         // * Mémorise n° de canal
    StarTimerCutOutMain();                                       // * Start Timer CutOut Main
  }
  else {                                                         // * Prog
    gpio_matrix_out(GPIO_NUM_33, 0x100, false, false);           // * Déconnecte la pin 33 du module RMT
    gpio_set_level(GPIO_NUM_33, 1);                              // * Pin 33 à l'état haut
    canalProg = channel;                                         // * Mémorise n° de canal
    StarTimerCutOutProg();                                       // * Start Timer CutOut Prog.
  }
#endif                                                           // * #endif
